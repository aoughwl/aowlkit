## Terminal presentation: truecolor ANSI, ANSI-aware padding, and an aligned
## table — the layer every aoughwl CLI otherwise reimplements.
##
## No dependencies beyond the stdlib. Colour is OFF for NO_COLOR, TERM=dumb and
## non-TTY output, ON when FORCE_COLOR is set, so piping any of these tools into
## a file gives plain text and a harness can still exercise the styled paths.
##
## The palette and glyph set are shared on purpose: `aowlup` and `aowlmony` are
## presented to a user as one toolchain, and two tables that align differently
